...
/**
 * Handles Flame Book projectile terrain impacts by replacing the impacted block with Volcanic Rock.
 *
 ...
 */
public final class FlameBookBlockImpactSystem extends TickingSystem<EntityStore> {

    private static final String FLAME_PROJECTILE_ID = FlameBookProjectileHitSystem.FLAME_PROJECTILE_ID;
    ...
    public FlameBookBlockImpactSystem(
    ...
    public void tick(float delta, int tickCount, @Nonnull Store<EntityStore> store) {
        try {
                    ...
                    for (int i = 0; i < chunk.size(); i++) {
                        ...
                        if (!Double.isFinite(length) || length <= 1e-6) {
                            ...
                            if (placed) {
                                debug.traceFileOnly(
                                    (PlayerRef) null,
                                    "FlameBook event=projectileImpact"
                                        ...
                                        + " volcanicBlockId=" + VOLCANIC_ROCK_BLOCK_ITEM_ID
                                        + " placed=true"
                                );
                        ...
                        int blockY = hit.y;
                        int blockZ = hit.z;
                        boolean placed = convertBlockAt(world, blockX, blockY, blockZ);
                        debug.traceFileOnly(
                            (PlayerRef) null,
                            "FlameBook event=projectileImpact"
                                + " projectileId=" + projectileAssetName
            ...
            errors.report((PlayerRef) null, "FlameBookBlockImpactSystem: tick failed.", t);
        }
    }
    ...
    private boolean convertBlockAt(@Nonnull World world, int x, int y, int z) {
